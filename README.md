...
| Banned APIs | `DateTime.Now`, `DateTimeOffset.Now`, `Task.Result`, `Task.Wait()`, `GetAwaiter().GetResult()`, `Thread.Sleep` |
...
Every standard MSBuild property the package sets uses a `Condition="'$(X)' == ''"` guard, so a value in your project always wins.
